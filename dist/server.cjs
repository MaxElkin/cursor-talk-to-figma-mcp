...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to modify"),
    property: import_zod.z.enum(["fills", "strokes"]).describe("Which paint property should use the style"),
    styleId: import_zod.z.string().optional().describe("The local paint style ID to apply"),
    styleName: import_zod.z.string().optional().describe("The local paint style name to apply. Used when styleId is not provided.")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to modify"),
    property: import_zod.z.enum(["fills", "strokes"]).describe("Which paint property should use the variable"),
    variableId: import_zod.z.string().optional().describe("The local color variable ID to bind"),
    variableName: import_zod.z.string().optional().describe("The local color variable name to bind. Used when variableId is not provided."),
    paintIndex: import_zod.z.number().int().min(0).optional().describe("The fill or stroke paint index to bind. Defaults to 0.")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().optional().describe("The ID of the node to modify. Required when target is node. Can also identify a page when target is page and pageId is omitted."),
    pageId: import_zod.z.string().optional().describe("The ID of the page to modify when target is page."),
    target: import_zod.z.enum(["node", "page", "current_page"]).optional().describe("Where to apply the variable mode. Defaults to node when nodeId is provided, otherwise current_page."),
    collectionId: import_zod.z.string().optional().describe("The local variable collection ID. Used instead of collectionName when provided."),
    collectionName: import_zod.z.string().optional().describe("The local variable collection name, for example M3. Used when collectionId is not provided."),
    modeId: import_zod.z.string().optional().describe("The variable collection mode ID. Used instead of modeName when provided."),
    modeName: import_zod.z.string().optional().describe("The variable collection mode name, for example Light or Dark. Used when modeId is not provided.")
  },
...
server.tool(
  ...
  "Move a node to a new position in Figma. Coordinates are local to the parent by default; use coordinateSpace='absolute' for canvas coordinates.",
  {
    ...
    coordinateSpace: import_zod.z.enum(["local", "absolute"]).optional().describe("Coordinate space for x/y. Defaults to local parent coordinates. Use absolute for canvas coordinates.")
  },
...
server.tool(
  ...
  "Rename a node in Figma",
  {
    ...
    name: import_zod.z.string().describe("The new node name")
  },
  async ({ nodeId, name }) => {
    try {
...
server.tool(
  ...
  "Create a new variant from an existing component. If x/y are omitted, the variant is placed below existing variants. If x/y are provided, coordinates are local to the component set by default; use coordinateSpace='absolute' for canvas coordinates.",
  {
    ...
    coordinateSpace: import_zod.z.enum(["local", "absolute"]).optional().describe("Coordinate space for x/y. Defaults to local component-set coordinates. Use absolute for canvas coordinates.")
  },
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).min(1).describe("Array of node IDs to move"),
    parentId: import_zod.z.string().describe("The ID of the target parent node"),
    index: import_zod.z.number().int().optional().describe("Optional insertion index in the new parent"),
    placement: import_zod.z.enum(["preserve_absolute", "inside_parent"]).optional().describe("How to place nodes after reparenting. Defaults to preserve_absolute.")
  },
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).min(1).describe("Array of node IDs to reorder"),
    position: import_zod.z.enum(["front", "forward", "backward", "back"]).describe("Layer-order action to apply")
  },
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).min(1).describe("Array of node IDs to group"),
    parentId: import_zod.z.string().optional().describe("Optional parent node ID for the new group. If omitted, uses the shared parent of the provided nodes."),
    index: import_zod.z.number().int().optional().describe("Optional insertion index for the new group in the parent."),
    name: import_zod.z.string().optional().describe("Optional name for the created group.")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the group node to ungroup")
  },
  async ({ nodeId }) => {
    try {
...
server.tool(
  ...
  {
    namePrefix: import_zod.z.string().optional().describe("Only include variables whose name starts with this prefix, for example Schemes/"),
    collectionName: import_zod.z.string().optional().describe("Only include variables from a collection with this exact name"),
    includeValues: import_zod.z.boolean().optional().describe("Whether to include per-mode color values. Defaults to false.")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("ID of the component instance"),
    properties: import_zod.z.record(import_zod.z.any()).describe("Object mapping component property names to values")
  },
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).describe("Array of component instance IDs to reset")
  },
  async ({ nodeIds }) => {
    try {
